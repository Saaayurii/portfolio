...
export const Contact = () => {
  ...
  const {0:formDetails, 1:setFormDetails} = useState(formInitialDetails);
  const {0:buttonText, 1:setButtonText}  = useState('Отправить');
  const {0:status, 1:setStatus}  = useState({});
  ...
  const handleSubmit = async (e) => {
    ...
    if (result.code == 200) {
      setStatus({ succes: true, message: 'Сообщение успешно отправлено'});
    } else {
      setStatus({ succes: false, message: 'Произошла ошибка, попробуйте позже'});
    }
  };
